...
// Same shape as routes/employees.js's POST /login response (minus branches,
// which GET /employees/me attaches separately), so req.employee can eventually
// replace req.body.employee_id trust throughout the routes.
async function loadEmployee(employeeId) {
  const { rows: [emp] } = await db.execute({
    sql: `SELECT e.id, e.first_name, e.last_name, e.username, e.role, e.must_change_password, e.security_group_id, e.default_branch_id, e.is_driver, e.is_operator, e.is_security, e.is_salesperson,
