...
const PERMISSION_TREE = [
  { key: 'dashboard', subs: [] },
  { key: 'pos', subs: [
    { key: 'pos_discounts' }, { key: 'pos_discount_override' }, { key: 'pos_refunds' }, { key: 'pos_void_items' }, { key: 'pos_hold' },
    { key: 'pos_customer_lookup' }, { key: 'pos_item_lookup' }, { key: 'pos_tax_exempt' }, { key: 'pos_reprint_ticket' },
    { key: 'pos_pay_on_account' }, { key: 'pos_change_quantity' }, { key: 'pos_cancel_ticket' }, { key: 'pos_payment' },
  ] },
  { key: 'drawers', subs: [{ key: 'drawers_manage' }, { key: 'drawers_open' }, { key: 'drawers_close' }, { key: 'void_transactions' }] },
  { key: 'inventory', subs: [
    { key: 'inventory_add' }, { key: 'inventory_edit' }, { key: 'inventory_delete' }, { key: 'inventory_adjust' },
    { key: 'inventory_print_labels' }, { key: 'inventory_merge' }, { key: 'inventory_export' },
    { key: 'inventory_missing_data' }, { key: 'inventory_template' }, { key: 'inventory_import' },
    { key: 'inventory_import_history' }, { key: 'inventory_scan_images' }, { key: 'inventory_categories' }, { key: 'inventory_brands' },
  ] },
  { key: 'customers', subs: [
    { key: 'customers_add' }, { key: 'customers_edit' }, { key: 'customers_delete' }, { key: 'customers_credit' },
    { key: 'customers_import' }, { key: 'customers_import_history' }, { key: 'customers_export' },
    { key: 'customers_export_balances' }, { key: 'customers_missing_rental_info' }, { key: 'customers_print_history' },
    { key: 'customers_rental_preapprove' },
  ] },
  { key: 'transactions', subs: [{ key: 'transactions_export' }, { key: 'transactions_refund' }, { key: 'transactions_credit_note' }, { key: 'transactions_returns' }] },
  ...
  { key: 'services', subs: [] },
  { key: 'rentals', subs: [
    { key: 'rentals_manage_items' }, { key: 'rentals_checkout' }, { key: 'rentals_returns' }, { key: 'rentals_issue' },
    { key: 'rentals_pause' }, { key: 'rentals_confirm_pickup' }, { key: 'rentals_deposit_refund' }, { key: 'rentals_credit_note' },
    { key: 'rentals_security_signoff' }, { key: 'rentals_confirm_delivery' },
  ] },
  ...
  { key: 'sales_manager', subs: [] },
  // Two independent top-level permissions, not parent/sub — creating a
  // Special Project and approving one are meant to be separable
  // (segregation of duties: whoever sets the markup shouldn't necessarily
  // be the one who can sign off on it). As a sub-key, checking the
  // special_projects module would auto-imply approval rights via the
  // bidirectional can() fallback; as siblings, each requires its own
  // explicit grant.
  { key: 'special_projects', subs: [] },
  { key: 'special_projects_approve', subs: [] },
  ...
  { key: 'commissions', subs: [{ key: 'commissions_plans' }, { key: 'commissions_approve' }, { key: 'commissions_pay' }, { key: 'commissions_delete' }] },
...
// Sub-permissions that must NOT bubble up to grant the parent `rentals`
// module via the bidirectional can() check above. Kept as a narrow,
// explicit carve-out (not a PERMISSION_TREE-wide behavior change) because
// these two subs are the only ones meant for a Driver-only security group:
// rentals_confirm_pickup/rentals_confirm_delivery must unlock nothing beyond
// GET /agreements/driver-queue and the confirm-pickup/claim-delivery actions
// — never the full agreement list/detail (financial fields), availability,
// deposits summary, or pause/resume. Mirrors App._canManageRentals() in
// public/index.html; keep both in sync.
const RENTALS_NON_ESCALATING_SUBS = new Set(['rentals_confirm_pickup', 'rentals_confirm_delivery']);

function canManageRentals(permissions) {
  if (!permissions) return false;
  if (permissions.rentals) return true;
  const mod = PERMISSION_TREE.find(m => m.key === 'rentals');
  return mod.subs.some(s => !RENTALS_NON_ESCALATING_SUBS.has(s.key) && !!permissions[s.key]);
}

function requireRentalsManage(req, res, next) {
  if (req.apiKey) return next();
  if (!req.employee) return res.status(401).json({ error: 'Authentication required' });
  if (!canManageRentals(req.employee.permissions)) {
    return res.status(403).json({ error: 'Missing permission: rentals' });
  }
  next();
}
...
module.exports = { PERMISSION_TREE, can, requireAuth, requirePermission, requireAnyPermission, canManageRentals, requireRentalsManage };
