...
function assertRentalCustomerEligible(customer) {
  ...
  if (!customer.rental_address_proof_path) missing.push('an uploaded proof of address document');
  if (!customer.rental_reference_name || !customer.rental_reference_phone) missing.push('the main reference (name and phone)');
  if (!customer.rental_reference_id_path) missing.push("a scanned ID for the main reference");
  if (!customer.rental_reference2_name || !customer.rental_reference2_phone) missing.push('a second reference (name and phone)');
  if (!customer.rental_reference3_name || !customer.rental_reference3_phone) missing.push('a third reference (name and phone)');
  if (missing.length) {
...
module.exports = { getBranchStock, feeFor, buildRentalLines, revalidateQuoteLines, insertPendingAgreement, assertRentalCustomerEligible, dueDateTime, requiredDepositReturnMethod };
